function(input, output, session) {
  ...
  output$overview_gps_refdate_text <- renderUI({
    ...
    tags$p(
      ...
      HTML(
        glue(
          "<strong>Reference date:</strong> {refdate}<br>",
          "<span class='text-muted small'>Previous {days_back} day",
          "{ifelse(days_back == 1, '', 's')} only; the reference date itself is excluded.</span>"
        )
      )
    )
  })

  output$overview_gps_observers_ui <- renderUI({
    choices <- overview_gps_heatmap_mock_observer_choices()

    pickerInput(
      inputId = "overview_gps_observers",
      label = "Mock users:",
      multiple = TRUE,
      choices = choices,
  ...
  overview_gps_days_back <- debounce(
    reactive({
      req(input$overview_gps_days_back)
      input$overview_gps_days_back
    }),
    200
  )

  overview_gps_selected_observers <- reactive({
    input$overview_gps_observers
  })

  output$overview_gps_heatmap_show <- renderLeaflet({
    try_else(
      overview_gps_heatmap_leaflet_mock(
        refdate = active_refdate(),
        days_back = overview_gps_days_back(),
        selected_observers = overview_gps_selected_observers()
      ),
      fallback_leaflet,
      fail = "overview_gps_heatmap_leaflet_mock() failed!"
    )
  })
